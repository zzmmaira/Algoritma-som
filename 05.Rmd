...
1.somgrid : initialized SOM node set
2.som : making a SOM model, change the radius of neighbourhood, learning rate, and iteration
3.plot.kohonen / plot : visualisation of resulting SOM
...
```{r}
library(readr)
iris <- read_csv("C:/Users/ASUS/Downloads/iris.csv")
View(iris)
```
...
t mencakup tiga spesies iris dengan masing-masing 50 sampel serta beberapa sifat dari setiap bunga. Satu spesies bunga dapat dipisahkan secara linear dari dua lainnya, tetapi dua lainnya tidak dapat dipisahkan secara linear satu sama lain.
